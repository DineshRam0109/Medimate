...
# Load model 
model_path ="C:\\Users\\dines\\OneDrive\\Documents\\Medicine-Recommendation-System\\model\\random_forest_model.pkl"
...
def helper(dis):
    ...
    pre= precautions[precautions['Disease'] == dis_lower][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values

    med = medications[medications['Disease'] == dis]['Medication'].values
    med = [m for m in med if pd.notna(m)]  # Filter out NaN medications

    die = diets[diets['Disease'] == dis]['Diet'].values
    die = [d for d in die if pd.notna(d)]  # Filter out NaN diets

    wrkout = workout[workout['disease'] == dis]['workout'].values
    wrkout = [w for w in wrkout if pd.notna(w)]  # Filter out NaN workouts
...
def get_predicted_value(patient_symptoms):
    ...
    # Set the corresponding indices to 1 for the symptoms provided by the user
    ...
    # Convert the input vector to a DataFrame to match the model's expected input format
...
def predict():
    if request.method == 'POST':
        ...
        # Split and process the user's symptoms input
        user_symptoms = [s.strip().lower() for s in symptoms.split(',')]
        
        # Get predicted disease
        predicted_disease = get_predicted_value(user_symptoms)
        
        # Retrieve additional information based on predicted disease
